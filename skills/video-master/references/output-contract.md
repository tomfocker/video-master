...
## Workflow Entry Mode
- Workflow mode: autopilot | guided
- Confirmation policy:
- Assumption policy:
- Why this mode:
...
## WebUI State Metadata

`qa/metadata/project_state.json` is generated by `scripts/project_state.py`. It is the read-only UI state summary for WebUI and other tools. It does not replace the canonical project files.

`scripts/serve_webui.py` serves the local WebUI and project APIs:

- `GET /api/projects`: list project packages under a project root.
- `GET /api/project?path=<project>&write=true`: build a project state response and optionally write `qa/metadata/project_state.json`.
- `GET /api/file?project=<project>&path=<relative_file>`: serve verified project-local frame files for visual inspection.
- `GET /api/auth/codex/status`: report whether a local Codex image-generation session is available.
- `POST /api/auth/codex/device/start`: start Codex device login for WebUI-native storyboard image generation.
- `POST /api/auth/codex/device/poll`: poll Codex device login and store tokens outside the project repo by default.
- `POST /api/storyboard-image/generate`: generate or regenerate a selected shot's storyboard image and write the original to `最终交付/01_分镜图/`.

`qa/metadata/image_generation_manifest.json` records WebUI-native image generation status. A successful shot entry includes `status`, `operation`, `model`, `size`, `quality`, `output_format`, `prompt`, `path`, and `generated_at`; failed entries include `status: failed` and `error`. This manifest is metadata for traceability and retry handling, not a user-facing final deliverable.
...
`qa/metadata/workflow_events.jsonl` records assumptions, user confirmations, generated-file updates, and UI change requests as append-only events. The first WebUI version may read this file when present and leave it empty otherwise.
