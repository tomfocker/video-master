...
For Seedance 2.0 or 15-second web video generation, also read `references/seedance2-practical-playbook.md` before writing image prompts or copy-ready video prompts.
...
动态时间切片：
(00-1.0s): <根据该分镜分析出的起势动作、景别、第一段运镜、角色口型或表情、SFX，不复用固定模板>。
(1.0-2.2s): <该分镜最重要的动作推进、道具/角色/环境互动、机位变化、对白口型或反应>。
(2.2-3.0s): <收束到本镜头的关键视觉点、表情/材质/产品细节、SFX 或转场方向>。
...
Time-slice count should follow duration: 2s can use 2 slices, 3-5s usually uses 3-4 slices, 10s uses about 8 slices, and 15s uses enough slices to cover the internal beats without overloading the prompt. Do not make every slice a new unrelated scene; the slices describe how one segment evolves over time. Each slice must be inferred from that specific storyboard: choose shot-specific framing, camera movement, action beats, prop interaction, environmental motion, character mouth cues, and SFX. Do not reuse generic phrases such as `gentle dolly 或 tabletop camera movement` or `镜头继续完成本镜头节拍`.
