...
class ReferenceStyleWorkflowTest(unittest.TestCase):
    ...
    def test_style_template_guardrails_are_documented(self):
        ...
        self.assertIn("do not paste that sentence into copy-ready model prompts", prompt_reference)
        self.assertIn("生成要求：", prompt_reference)
        self.assertIn("音频：背景音乐不要生成", prompt_reference)
        self.assertIn("Do not add standalone `运镜与焦段` or `光线与风格` lines", prompt_reference)
    ...
    def test_seedance2_is_documented_as_default_video_prompt_profile(self):
        ...
        self.assertIn("(00-1s)", prompt_reference)
        self.assertIn("动态时间切片", prompt_reference)
        self.assertIn("Time slices should be rhythm-driven", prompt_reference)
        self.assertIn("whole-second ranges", prompt_reference)
        self.assertIn("同期声", prompt_reference)
        self.assertIn("sync_sound_policy", output_contract)
